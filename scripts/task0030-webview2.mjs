...
assert(/^task0030-[a-f0-9]+$/.test(variant));
let target;
for(let n=0;n<100;n++) {
...
await evaluate(`window.task0030Keys=[];document.addEventListener('keydown',e=>window.task0030Keys.push({key:e.key,trusted:e.isTrusted}),true)`);
...
const keys=await evaluate("window.task0030Keys");assert(keys.length>0 && keys.every(k=>k.trusted));
...
const artifact={task:"TASK-0030",status:"NONCANONICAL_ENGINEERING_EVIDENCE",engine:"Windows Tauri / real WebView2",webviewVersion:host.webviewVersion,tauriVersion:host.tauriVersion,sqliteVersion:host.sqliteVersion,source:"normal map_open scan -> canonical Index -> map_view -> MapApp/MapView",small,large,progressiveNavigation:{pageReplaced:true,noDuplicatePageMembers:true,domMatchesProductPage:true},trustedKeydowns:keys.length,fatalConsoleErrors:fatal.length,sourceArtifacts:after.filetopoArtifacts,limits:["Development workstation; not a modest-laptop acceptance test", "No GPU-disabled claim", "100k product DTO is covered by Rust and jsdom; this WebView2 pass uses 6001 physically scanned synthetic nodes", "Gesture timings include deliberate CDP settling waits; not renderer latency benchmarks"]};
await writeFile("docs/performance/runs/TASK-0030-webview2.json",JSON.stringify(artifact,null,2)+"\n");
