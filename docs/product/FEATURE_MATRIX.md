...
| F-036 | Accessibilité | Clavier, contraste, alternatives | **Fondations déjà présentes dans le runtime MapApp** : arbre SVG sémantique + clavier (`role=tree/treeitem`, `aria-activedescendant`), agrégats clavier, menu CompositionBar clavier, labels/alertes ARIA, focus visible, états avec texte/symbole/contour et `prefers-reduced-motion` | `ACTION-0078`; `src/map/MapView.tsx`; `CompositionBar.tsx`; `map.css`; panneaux MapApp | **Fermeture non prouvée** : aucun audit axe/WebView2 global publié avant TASK-0047, aucun parcours clavier complet sans piège, contrastes et alternatives non colorées non inventoriés systématiquement. `TASK-0047` READY sous `DEC-0045`, axe-core local/dev-only + preuve WebView2 | P1 | 8 | PROPOSED | Audit automatisé WebView2 + parcours clavier/focus + contrastes + non-couleur + reduced motion, puis contrôle indépendant | `MVP` |
