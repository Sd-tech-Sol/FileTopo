...
| F-033 | Personnalisation | Nom, couleur, icône, préférences | Backend/catalogue déjà capable de modifier et persister nom/couleur/icône (`map_brain_update`); le runtime `MapApp` affiche ces métadonnées mais **n'offre aucun contrôle utilisateur pour les modifier** | `TASK-0018` **VERIFIED** (`ACTION-0029`) pour le modèle, l'isolation et la persistance; audit post-`TASK-0044` : aucun appel `map_brain_update` dans `MapApp.tsx` | Édition utilisateur absente du runtime V1 courant; c'est le dernier écart nommé de `P-20` après la reprise brain-scoped de `TASK-0044` | P1 | 7 | PROPOSED | Valeurs éditables et persistantes | `MVP` |
...
| F-035 | FR/EN | Interface bilingue persistante | **Prototype historique seulement** : `src/App.tsx` + `src/lib/locale.ts` ont FR/EN; le runtime V1 courant `src/map/MapApp.tsx` force `strings.fr`, `document.documentElement.lang = \"fr\"` et plusieurs panneaux restent français | Audit post-`TASK-0044` / `ACTION-0073`; `SourceObservationBadge` et `WatchStatusBadge` ont déjà des dictionnaires FR/EN réutilisables, ainsi que `ContentObservationsPanel`; aucune preuve bilingue du runtime `MapApp` | Le statut historique `IMPLEMENTED` était périmé pour le runtime courant. Choix de langue et persistance V1 manquent; `P-19` et `P-21` restent partiels | P1 | 8 | PROPOSED | Tests des deux langues et repli | `MVP` |
