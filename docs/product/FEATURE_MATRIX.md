...
| F-005 | Exclusions | Règles sûres, visibles et configurables | Politique V1 brain-scoped/versionnée de sous-arbres relatifs exacts; visible FR/EN; backend autoritaire; commune scan/refresh/rebuild/W-B/W-C/watcher; reparse/symlink toujours fermés; rebase sans faux événements source | `scanner.rs`; `map/exclusion_policy.rs`; `ExclusionsPanel.tsx`; `DEC-0046`; `TASK-0048` IMPLEMENTED; artefact WebView2 | Trois cerveaux dont A/C sur la même source, politiques isolées et persistées; source absente/restaurée et watcher prouvés; contrôle indépendant encore requis | P0 | 2 | IMPLEMENTED | Exclusions testées et explicables | `MVP` |
