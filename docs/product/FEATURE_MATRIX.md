...
| F-002 | Cerveau indépendant | Racine, index et état isolés | Présent mais incomplet | registry.rs:60; lib.rs:229 | **Reprise par cerveau `IMPLEMENTED` par `TASK-0044`** (branche, sélection, caméra, filtre, panneau : un enregistrement par cerveau dans le catalogue, `TASK-0044-webview2.json`), **en attente de contrôle indépendant**; restent hors portée : langue, accessibilité, composition multi-cerveaux persistante | P0 | 3 | PROPOSED | Deux cerveaux ne partagent aucun état | `MVP` |
...
| F-034 | Plusieurs cerveaux | Sélection et chargement indépendants | Onglets enregistrés | App.tsx:148,310 | **Redémarrage et bascule restaurent chaque cerveau** (`TASK-0044` `IMPLEMENTED`, trois cerveaux et deux redémarrages réels), **en attente de contrôle indépendant**; une composition de plusieurs cerveaux reste session seule | P0 | 7 | PROPOSED | Redémarrage et bascule restaurent chaque carte | `MVP` |
